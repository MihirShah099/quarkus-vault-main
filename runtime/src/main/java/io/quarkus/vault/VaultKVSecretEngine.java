...
public class VaultKVSecretEngine {
    ...
    /**
     * Deletes the secret metadata and all the version data at the given path. It
     * has no effect if no secret is currently stored at path.
     * <p>
     * For KV v1 secret engine, this method is same as {@link #deleteSecret(String)}
     *
     * @param path to delete
     */
    public void deleteSecretMetadata(String path) {
        engine.deleteSecretMetadata(path).await().indefinitely();
    }
